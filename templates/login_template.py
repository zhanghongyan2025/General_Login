...
class LoginPage:
    def __init__(self, page: Page):
        ...
        self.{{ elem.var_name }} = {{ elem.locator }}  # 移除外层引号
        {% endfor %}

    {% for elem in elements if elem.type == "input" %}
    def fill_{{ elem.var_name }}(self, value: str, test_fields: str = None):
        \"\"\"填写{{ elem.var_name }}输入框\"\"\"
        if test_fields:
            pass
        self.{{ elem.var_name }}.fill(value)  # 直接调用定位器的fill方法
    {% endfor %}

    {% for elem in elements if elem.type == "button" %}
    def click_{{ elem.var_name }}(self, test_fields: str = None):
        \"\"\"点击{{ elem.var_name }}按钮\"\"\"
        if test_fields:
            pass
        self.{{ elem.var_name }}.click()  # 直接调用定位器的click方法
    {% endfor %}
