...
def generate_login_page(elements: list, url, output_file: str = "pages/login_page.py"):
    ...
    import conf
    importlib.reload(conf.scenarios_setting)
    template = Template(LOGIN_PAGE_TEMPLATE)
    # 渲染模板时传入elements和TEST_FIELDS
    code = template.render(
        elements=elements,
        url=url,
        TEST_FIELDS=conf.scenarios_setting.TEST_FIELDS
    )
...
def generate_testcase(target_url, output_path: str = "test_suits/test_login.py"):
    ...
    import conf
    importlib.reload(conf.scenarios_setting)
    locked_count, _ = extract_lock_info(conf.scenarios_setting.TEST_ERROR_MESSAGES["lock_account"].get("locked_user", ""))
    render_data = {
        "LOCKED_COUNT": locked_count,
        "TARGET_URL": target_url,
        "TEST_SCENARIOS": conf.scenarios_setting.TEST_SCENARIOS,
        "TEST_FIELDS": conf.scenarios_setting.TEST_FIELDS,
        "TEST_ERROR_MESSAGES": conf.scenarios_setting.TEST_ERROR_MESSAGES,
        "TEST_ACCOUNTS": conf.scenarios_setting.TEST_ACCOUNTS
    }
